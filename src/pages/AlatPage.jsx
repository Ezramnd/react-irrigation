...
import { FaFan, FaThermometerHalf, FaPowerOff, FaQrcode, FaImage, FaCrosshairs, FaTint,  FaBolt,  FaClock,  FaCog, FaSave, FaInfoCircle, FaExclamationTriangle, FaChartLine, FaHandPaper, FaCheckCircle, FaExclamationCircle} from 'react-icons/fa';
...
const ModalFormJadwalClimate = ({ onSave, onClose, jadwalToEdit }) => {
    ...
    const handlePilihSemuaFans = () => { 
        const semuaFans = [1, 2]; // Hanya ada Exhaust Fan dan 2
...
const ModalKontrolDosing = ({ alat, onClose, onEdit }) => {
 ...
 // 1. Initial Data Fetch & Socket Connection
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            ...
            try {
                ...
                if (resSettings.data) {
                    // Helper untuk parsing jam dari format HH:MM:SS
                   const parseHour = (t) => t ? String(parseInt(t.split(':')[0], 10)) : '';
    ...
    const handleSettingChange = (e) => {
        ...
        } else if (name === 'startTime_hour' || name === 'endTime_hour') {
        // Khusus jam, simpan sebagai string jam ("8", "22", atau "0" untuk 24:00)
        // Jika value adalah '0' (24:00), tetap simpan '0'. Jika '', simpan ''
                                            ...
                                            <div className="md:col-span-2 bg-purple-50 border border-purple-200 p-5 rounded-lg">
                                                ...
                                                <div className="grid grid-cols-2 gap-3">
                                                    <div>
                                                        ...
                                                        {/* settings.startTime_hour sudah string dari API / handleSettingChange */}
                                                    ...
                                                    <div>
                                                        ...
                                                        {/* settings.endTime_hour sudah string dari API / handleSettingChange */}
                                                        <select name="endTime_hour" value={settings.endTime_hour || ''} onChange={handleSettingChange} className="w-full p-2 border border-purple-300 rounded-lg bg-white font-semibold text-gray-800 focus:ring-2 focus:ring-purple-500 focus:outline-none">
                                                            <option value="">Pilih</option>
                                                            {hoursOptions}
                                                            {/* PERBAIKAN: Ubah value={0} menjadi value="0" (String) */}
