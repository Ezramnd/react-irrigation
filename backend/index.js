...
app.use(cors({
    credentials: true,
    origin: [
        FRONTEND_URL,"http://192.168.1.100:5173", "http://localhost:5173", "http://192.168.1.100:8081", "http://localhost:8081", "http://localhost:5000"
    ...
    if (topicStr.endsWith("/status") && topicStr.startsWith(`${mqttOptions.username}/esp32/alat/IRRIGATION-`)) {
       ...
       try {
            ...
            if (dbStatus === "inactive") {
                updateStatusData.lastOfflineAt = nowStatus;
            }

            if (dbStatus === "active") {
                updateStatusData.lastOnlineAt = nowStatus;
            }
