...
import agrifamLogo from '../assets/agrifam.jpg'; // Pastikan path logo ini benar
import api from '../api'; // DIGANTI: Menggunakan instance API terpusat dari kode kedua

// --- Komponen Ikon Mata (dari kode pertama, tidak diubah) ---
...
const LoginPage = () => {
    ...
    const [isLoading, setIsLoading] = useState(false); // DIPERTAHANKAN: dari kode pertama untuk UI loading
    const [keepLoggedIn, setKeepLoggedIn] = useState(false); // DIAMBIL: dari kode kedua untuk fungsionalitas
    
    // FUNGSI HANDLELOGIN DIGABUNGKAN
    const handleLogin = async (e) => {
        e.preventDefault();
        setIsLoading(true); // UI Loading dari kode pertama
        setErrorMsg('');
        try {
            // Logika dari kode kedua (menggunakan API dan keepLoggedIn)
            const response = await api.post('/login', { email, password, keepLoggedIn });
            
            // Simpan token ke localStorage
            localStorage.setItem('token', response.data.accessToken);
            
            // Navigasi dengan sedikit delay untuk UX
    ...
    // Varian animasi dari kode pertama (tidak diubah)
    const panelVariants = {
    ...
    return (
        // TAMPILAN UTAMA: Menggunakan layout dari kode pertama
        <div className="flex flex-col lg:flex-row min-h-screen font-sans bg-gray-50">
            {/* ================================== */}
            {/* PANEL KIRI/ATAS (DEKORATIF)      */}
            {/* ================================== */}
            <motion.div
                ...
                >
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        <motion.div variants={itemVariants}>
                            ...
                            <input type="email" id="email" className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="info@agrifam.link" value={email} onChange={(e) => setEmail(e.target.value)} required />
                        </motion.div>

                        <motion.div variants={itemVariants} className="relative">
                            <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="password">Password</label>
                            <input type={showPassword ? 'text' : 'password'} id="password" className="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-green-500 pr-12" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required />
                            <button type="button" className="absolute inset-y-0 right-0 top-6 flex items-center px-4 text-gray-500 hover:text-green-600" onClick={() => setShowPassword(!showPassword)}>
                                <EyeIcon isOpen={showPassword} />
                            </button>
                        </motion.div>
                      
                        <motion.div variants={itemVariants} className="flex justify-between items-center text-sm">
                            {/* CHECKBOX: Menggunakan fungsionalitas dari kode kedua */}
                        ...
                        <motion.div variants={itemVariants}>
                            {/* TOMBOL: Menggunakan UI loading dari kode pertama */}
                            <motion.button type="submit" disabled={isLoading} className="w-full p-3 bg-green-600 text-white rounded-lg font-bold shadow-lg shadow-green-500/30 hover:bg-green-700 transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                                {isLoading ? (
                                  <>
